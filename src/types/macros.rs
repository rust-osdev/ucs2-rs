...
macro_rules! cstr8 {
    () => {{
        ...
        unsafe { $crate::types::CStr8::from_bytes_with_nul_unchecked(S) }
    }};
    ($s:literal) => {{
        ...
        const NUM_CHARS: usize = $crate::types::str_num_latin1_chars($s) + 1;

        const VAL: [u8; NUM_CHARS] = $crate::types::str_to_latin1($s);

        // SAFETY: the `str_to_latin1` function always produces a valid Latin-1
        // string with a trailing null character.
        unsafe { $crate::types::CStr8::from_bytes_with_nul_unchecked(&VAL) }
    }};
}
...
macro_rules! cstr16 {
    () => {{
        ...
        unsafe { $crate::types::CStr16::from_u16_with_nul_unchecked(S) }
    }};
    ($s:literal) => {{
